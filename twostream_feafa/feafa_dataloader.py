...
#                     #if "_data" in fullpath3_temp: # remove if statements
#                     fullpath3.append(fullpath3_temp) # removed indent
...
class FeafaDataset(Dataset):
    def __init__(self, path, window,usage,train=0.8):
        ...
        #self.allwindows_frames,self.allwindows_labels = windowmapper([self.videopaths[0]],self.window) # THIS HAS BEEN MODIFIED FOR TESTING PURPOSES TO SHOW ONE VIDEO ONLY. CHANGE IT 
        self.allwindows_frames = windowmapper(self.videopaths,self.window) # THIS HAS BEEN MODIFIED FOR TESTING PURPOSES TO SHOW ONE VIDEO ONLY. CHANGE IT BACK IF YOU WANT TO TRAIN ON THE ENTIRE DATASET
